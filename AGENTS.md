...
- Each accepted submission fires one best-effort ntfy publish (phone push to the secret `NTFY_TOPIC` on the self-hosted server at `NTFY_URL`) to alert the moderator; it runs via `waitUntil` and must never delay or fail the submission itself.
- If `OPENAI_API_KEY` is configured, GPT-6 Luna approves or declines clear pending submissions automatically and leaves uncertain ones pending for the admin. Public and admin board reads retry missed reviews. Decisions and reasons stay in the admin queue. Model failures leave submissions pending, and a late AI result never overrides an admin status change.
