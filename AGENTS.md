...
- The board is anonymous by design: no accounts, votes deduplicated by a first-party cookie plus salted IP-hash rate limits, submissions gated by honeypot + Turnstile and held as `pending` until approved by GPT-6 Luna or an admin. Board user content is always rendered with `textContent`, never `innerHTML`.
- `TURNSTILE_SECRET`, `ADMIN_TOKEN`, `VOTE_SALT`, `NTFY_TOPIC`, `NTFY_TOKEN`, `NTFY_URL` (the self-hosted ntfy origin), and `OPENAI_API_KEY` are Pages secrets (`wrangler pages secret put`). Never commit or print them; the only committed Turnstile value is the public site key in `wrangler.toml`.
- Each accepted submission schedules a best-effort ntfy publish (phone push to the secret `NTFY_TOPIC` on the self-hosted server at `NTFY_URL`) after the review outcome is known: approved, denied, needs review, or review pending. A successful retry after a failure sends a final follow-up. Publishing runs via `waitUntil` and must never delay or fail the submission itself.
- If `OPENAI_API_KEY` is configured, GPT-6 Luna approves or declines clear pending submissions automatically and leaves uncertain or partially overlapping ideas pending for the admin. A duplicate is declined only when an existing item covers the whole request. Public and admin board reads retry missed reviews. Decisions and reasons stay in the admin queue. Model failures leave submissions pending, and a late AI result never overrides an admin status change.
