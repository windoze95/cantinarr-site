...
- `TURNSTILE_SECRET`, `ADMIN_TOKEN`, `VOTE_SALT`, `NTFY_TOPIC`, `NTFY_TOKEN`, and `NTFY_URL` (the self-hosted ntfy origin) are Pages secrets (`wrangler pages secret put`). Never commit or print them; the only committed Turnstile value is the public site key in `wrangler.toml`.
