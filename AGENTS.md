...
- `TURNSTILE_SECRET`, `ADMIN_TOKEN`, `VOTE_SALT`, `NTFY_TOPIC`, and `NTFY_TOKEN` are Pages secrets (`wrangler pages secret put`). Never commit or print them; the only committed Turnstile value is the public site key in `wrangler.toml`.
- Each accepted submission fires one best-effort ntfy publish (phone push to the secret `NTFY_TOPIC`) to alert the moderator; it runs via `waitUntil` and must never delay or fail the submission itself.
