...
- `TURNSTILE_SECRET`, `ADMIN_TOKEN`, `VOTE_SALT`, and `NTFY_TOPIC` are Pages secrets (`wrangler pages secret put`). Never commit or print them; the only committed Turnstile value is the public site key in `wrangler.toml`.
